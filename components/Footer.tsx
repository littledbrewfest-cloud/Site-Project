      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-10">
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-extrabold text-xl text-slate-900 dark:text-white tracking-wider uppercase">
                The ARC Raiders <span className="text-amber-600 dark:text-amber-400 font-mono text-xs px-1.5 py-0.5 rounded bg-amber-500/10 border border-amber-500/30">HUB</span>
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <Link
                href="/rss.xml"
                target="_blank"
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl bg-slate-100 dark:bg-slate-900 text-amber-600 dark:text-amber-400 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors border border-slate-200 dark:border-slate-800"
              >
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              <li>
                <Link href={`/category/${categoryToSlug("PS5 & Console Gaming")}`} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors">
                  PlayStation 5 Hub
                </Link>
              </li>
              <li>
                <Link href={`/category/${categoryToSlug("PC Specs & Performance")}`} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors">
                  PC System Requirements
                </Link>
              </li>
              <li>
                <Link href={`/category/${categoryToSlug("Weapons & Loadouts")}`} className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors">
                  Weapons & Armory
                </Link>
              </li>
              <li>
                <Link href="/robots.txt" target="_blank" className="hover:text-amber-600 dark:hover:text-amber-400 transition-colors">
                  Robots Index
                </Link>
              </li>
            </ul>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-col gap-2 text-xs">
              <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800">
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Lumen, Nanite & Chaos Physics</p>
              </div>
              <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800">
                <span className="font-semibold text-slate-900 dark:text-slate-200">Embark Studios</span>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">PvPvE Extraction Survival</p>
              </div>
            </div>
          </div>
        </div>

        <div className="mt-12 pt-8 border-t border-slate-100 dark:border-slate-900 flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500">
